    ...
    <Card sx={{ maxWidth: 500, m: 2, boxShadow: 3, listStyle: 'none', padding: 0 }}>
      <CardActionArea>
        <CardContent sx={{ '& .MuiTypography-root': { mb: 1 },  }}>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ display: 'flex', alignItems: 'center' }}>
            <DateRangeIcon color="action" sx={{ mr: 0.5 }} /> Expiration Date: {expirationDate}
          </Typography>
          <Typography variant="body2" sx={{ color: inspected ? green[500] : red[500], display: 'flex', alignItems: 'center' }}>
            {inspected ? <CheckCircleOutlineIcon sx={{ mr: 0.5 }} /> : <ErrorOutlineIcon sx={{ mr: 0.5 }} />} Inspected: {inspected ? 'Yes' : 'No'}
          </Typography>
        </CardContent>
      </CardActionArea>
    </Card>
